...
def main():
    ...
    table_names = ['book', 'campaign', 'constant', 'incentivized_action', 'offer', 'campaign', 'journey', 'campaign_run_debt', 'customer_reached','contact','transaction','event']

    # Specify the output folder
    output_folder = '/Users/ronen_saviz/PycharmProjects/erd_division_into_centarl_tables/Output'
    ...
    for table_name in table_names:
        ...
        # Write the output to a separate .dot file in the specified folder
        output_file_path = os.path.join(output_folder, f'{table_name}_graph.dot')
        with open(output_file_path, 'w') as file:
            file.write(graph_str)

        # Print the output to the console
        print(f'Graph for {table_name} written to {output_file_path}')
